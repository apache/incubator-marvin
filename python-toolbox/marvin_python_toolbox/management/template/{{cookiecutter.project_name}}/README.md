# {{cookiecutter.project_package}} v0.0.1
...
```
mkvirtualenv {{cookiecutter.project_package}}_env
```
...
The default log level is set to _WARNING_. You can change the log level at runtime setting another value to one of the following environment variable: `{{cookiecutter.project_package}}_LOG_LEVEL` or `LOG_LEVEL`. The available values are _CRITICAL_, _ERROR_, _WARNING_, _INFO_ and _DEBUG_.
